...
def drawReportChart(inFile,prefix):
    ...
    fastq_cnt    = pd.DataFrame({'total':dataset['Total reads'],'label':'Total Read','barcode':dataset['SampleID']})
    nan_filt     = pd.DataFrame({'total':dataset['Nanofilt count'],'label':'Nanofilt count','barcode':dataset['SampleID']})
    human_un     = pd.DataFrame({'total':dataset['Human unmapped'],'label':'Human unmapped','barcode':dataset['SampleID']})
    ...
    g = sns.catplot(x="label",y="total", col="barcode", col_wrap=4,
                data=df,
                kind="bar", height=2.5, aspect=1,sharey=False)
